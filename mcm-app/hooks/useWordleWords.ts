...
export default function useWordleWords() {
  ...
  const getWordForDate = (dateKey: string, cycle: 'morning' | 'evening'): string => {
    ...
    if (!loading && words[dateKey] && words[dateKey][cycleIndex]) {
      console.log(`🎯 Palabra obtenida de Firebase para ${dateKey} (${cycle}):`, words[dateKey][cycleIndex]);
    ...
    if (localWords[dateKey] && localWords[dateKey][cycleIndex]) {
      console.log(`📄 Palabra obtenida de wordle-daily.json para ${dateKey} (${cycle}):`, localWords[dateKey][cycleIndex]);
    ...
    if (availableDates.length > 0) {
      ...
      const randomWord = localWords[randomDate][cycleIndex] || localWords[randomDate][0];
      console.log(`🔄 Usando palabra aleatoria de wordle-daily.json: ${randomWord} (de ${randomDate})`);
      return randomWord;
    }
