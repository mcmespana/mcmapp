...
## 2026-05-13 — Revisión profunda: bugs, fugas de memoria y rendimiento

- **Pérdida de foco en la barra de búsqueda del cantoral**: en `SongListScreen` el `ListHeaderComponent` se declaraba como `const ListHeader = () => (...)` y se pasaba como `<ListHeader />`, lo que generaba un nuevo tipo de componente en cada render del padre. FlatList desmontaba/remontaba el header en cada tecla y el `SearchField` perdía el foco. Ahora se pasa como elemento JSX (no como componente función) y `renderItem` se mueve a `useCallback`. Archivos: `app/screens/SongListScreen.tsx`.
- **Fuga de listener de Firebase en notificaciones**: `subscribeToNotifications` devolvía `() => off(notificationsRef, 'value', unsubscribe)`, donde `unsubscribe` es la función de cleanup devuelta por `onValue`, no la callback original — `off()` no eliminaba el listener y se acumulaban suscripciones tras cada montaje del provider. Se reemplaza por la cleanup que devuelve `onValue`. Archivos: `services/pushNotificationService.ts`.
- **Crash silencioso por caché corrupta**: `useFirebaseData` y `useCalendarEvents` hacían `JSON.parse(cache)` sin protección; si la caché de AsyncStorage quedaba a medio escribir (cierre forzado) el throw rompía la carga en cada arranque. Ahora se captura el error, se limpia la caché afectada y se reintenta limpio. Archivos: `hooks/useFirebaseData.ts`, `hooks/useCalendarEvents.ts`.
- **Eventos multi-día con DST**: el iterador del calendario usaba `new Date(startStr)` + `setDate(+1)`, vulnerable a saltos/duplicados al cruzar el cambio de hora. Se reescribe usando milisegundos UTC + un guard de iteraciones (máx. 5 años) para garantizar resultados estables. Archivos: `hooks/useCalendarEvents.ts`.
- **Re-renders en cascada por contextos**: los providers `SettingsContext`, `AppSettingsContext`, `UserProfileContext` y `NotificationsContext` construían el `value` inline en cada render, forzando re-render de TODOS los consumidores. Se envuelve en `useMemo` y los setters en `useCallback`. En `SettingsContext` además se quita la doble escritura a AsyncStorage en cada cambio (el effect de guardado + el cleanup del effect de "guardar al desmontar" duplicaban la escritura). Archivos: `contexts/{SettingsContext,AppSettingsContext,UserProfileContext,NotificationsContext}.tsx`.
- **`SongListItem` reactivado por contexto entero**: el `animatedStyle` y los handlers se recreaban en cada render; se memoizan con `useMemo`/`useCallback`. Archivos: `components/SongListItem.tsx`.
- **Parser BBCode**: el helper `replaceIteratively` reaplicaba cada regex en bucle `do/while` aunque el flag `g` ya reemplaza todas las apariciones en una sola pasada — además era vulnerable a entradas que no convergiesen. Se simplifica a un único `replace` por regla con guard de tamaño máximo de entrada para evitar amplificaciones DoS. Se ajustan los grupos para no engullir corchetes en atributos (`[^\]]*?`). Archivos: `utils/formatText.ts`.
- **Rechazo no manejado en banner PWA**: `installEvent.userChoice.finally(...)` no atrapaba un eventual reject de la promesa (UnhandledPromiseRejection en navegador). Se envuelve en `Promise.resolve(...).catch().finally()`. Archivos: `components/AddToHomeBanner.tsx`.
