...
## 2024-05-18 - Early exit on Array Iteration
**Learning:** Functions that only need a small chunk of items from a large dataset should not use chained array methods (`filter().sort().forEach()`), as these methods allocate intermediate arrays and process the entire dataset before returning a result. This can cause significant overhead, particularly in frequently called memoized selectors (e.g., finding just the next 2 events).
**Action:** Use native `for` or `for...of` loops, and return early out of the loops when the target number of items is reached to bypass processing all items in large lists.
