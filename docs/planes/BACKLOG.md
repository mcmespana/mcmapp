...
| **⚠️ Roto y sin dueño**      | 🚨 **Android no muestra las notificaciones con categoría** desde el 2026-08-03: el Panel manda `channelId` y la app de `production` no tiene esos canales. Decisión del usuario: OTA a `production` o que el Panel mande `default` (`mcm-app/TODO.md`, Notificaciones) |
